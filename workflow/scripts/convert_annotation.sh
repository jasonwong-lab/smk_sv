...
vcf2maf.pl --input-vcf "${input_vcf2maf}" --output-maf "${maf}" --inhibit-vep --ncbi-build "${genome}" --cache-version "${version_cache_vep}" --ref-fasta "${fasta}" --vcf-tumor-id "${sample}" --tumor-id "${sample}" --vep-data "${dir_db_vep}" --species homo_sapiens
